...
#Destination Directory select function
...
def work_with_files(self):
    ...
    for f in fList:
        if f.endswith('.txt'):
            txt_files_source = os.path.join(self.source_dir,f) #Absolute path of txt file in the source directory
            txt_files_dest = os.path.join(self.dest_dir,f) #Absolute path of txt file in the destination directory

            #Trasfer the file into destination directory
